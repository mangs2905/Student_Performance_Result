...
class reportClass:
    def __init__(self,root):
        ...
        btn_print = Button(self.root, text="Print_Report", font=("goudy old style", 15, "bold"), bg="red", fg="white", cursor="hand2", command=self.print_report).place(x=650, y=350, width=150, height=35)
...
#============================================================================================================
    ...
    def delete(self):
        ...
        try:
            if self.var_id=='':
                ...
                cur.execute("select * from course where rid=?", self.var_course.get(),)
                row= cur.fetchone()
                if row ==None:
                    ...
                    if op==True:
                        cur.execute("delete from result where rid=?",(self.var_course.get(),))
                        con.commit()
    ...
    def print_report(self):
        student_roll = self.roll.cget("text")
        student_name = self.name.cget("text")
        course = self.course.cget("text")
        marks = self.marks.cget("text")
        total_marks = self.full_marks.cget("text")
        percentage = self.per.cget("text")

        print("Student Roll No:", student_roll)
